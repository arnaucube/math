...
\begin{proof}
    ...
    Let $x \in \Ll^\perp$. By definition $Ax=0 \pmod q$, therefore $\exists~ k \in \Zz^n$ such that $Ax = qk$.
    ...
    Let $\hat{x} \in ker(A) \subseteq \Zz_q^m$.
    Since $A \hat{x}=0$, then $Ax =0 \pmod q$, thus $x \in \Ll^\perp$.

    Since $y \in \Ll^{\perp *}$, $\langle y, x \rangle \in \Zz$. Multiply it by $q$,
    $$\langle qy, x \rangle = q \langle y, x \rangle \in q \Zz$$
    ...
    which means that $qy \in \Ll$ (by definition of $\Ll$). Divide that by $q$, obtaining
    $$y \in \frac{1}{q} \Ll$$
