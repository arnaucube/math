...
% \vspace*{-1.8cm}
  ...
  $$\text{public:}~A \leftarrow R_q^{k\times\ell},~ t = As_1+s_2$$
  $$y_1\leftarrow Y^\ell,~ y_2\leftarrow Y^k$$
  $$w = Ay_1+y_2$$
...
\vspace*{2.5cm}
  \begin{align*}
    w &\stackrel{?}{=} Az_1+z_2-ct \\
      &\text{indeed,}\\
      &= A(y_1+cs_1)+(y_2+cs_2) \\
    &~-c(As_1+s_2) \\
    =A &y_1 +c A s_1 +y_2 +c s_2-cAs_1-cs_2\\
    &= Ay_1+y_2 = w
  \end{align*}
...
\begin{frame}{Conclusions - Key Takeaways}
  \begin{itemize}
    \item no tool for all use cases (eg. Ethereum validators vs wallets)
    \item lattice-based cryptography:
    \begin{itemize}
      \item not a single assumption on all schemes: NTRU, SIS, R-SIS, M-SIS, LWE, R-LWE, M-LWE, $\ldots$
      \item actual operations (implementation) are over polynomial rings
      \item smaller proofs and keys than hash-based
      \item often we're interested in 'short' values $\longrightarrow$ trick: decompose bigger values before operate on them
    \end{itemize}
    \pause
  ...
  -\\
  \href{https://arnaucube.com}{arnaucube.com}\\
  $\mathbb{X}$ \href{https://x.com/arnaucube}{$@$arnaucube}
\end{frame}
