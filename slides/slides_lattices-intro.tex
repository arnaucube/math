...
\author{
\scriptsize{September 2026, Barcelona}
}
\date{
  \scriptsize{
  \href{https://arnaucube.com}{arnaucube.com}\\
  \href{https://x.com/arnaucube}{$@$arnaucube}
}
}
...
% [done] Lattices are already used in browsers: explain how, hybrid signatures, etc
% clarify L^\perp notation, is it q-ary?
...
\begin{frame}{Motivation}
  \begin{itemize}
    ...
    \item even if quantum computers never reach, the fact that there is a threat, forces us to prepare for it
    \item real-world example: since 2023/2024 Chrome and Firefox have lattice-based cryptography to the browser for TLS
    \begin{itemize}
...
\begin{frame}{Motivation (in Ethereum's ecosystem)}
  \begin{itemize}
    ...
    \item hash-based schemes don't have much algebraic structure
      \begin{itemize}
	\item good for less attacks, not so fun to build stuff with (less room to compose)
...
\begin{frame}{First, some tools}
  \centering
  \includegraphics[width=8.0cm]{./zelda-dangerous-out-there}
\end{frame}

\begin{frame}{Recap of concepts}
  \begin{itemize}
    \item \textbf{Ring:} a set with addition and multiplication, where addition forms an abelian group and multiplication is associative and distributive over addition.
    \begin{itemize}
      \item \textbf{Ideal:} a subset $I \subseteq R$ closed under addition and under multiplication by arbitrary elements of $R$.
      \scriptsize{
      \colorbox{gray!20}{Intuition:
	$I$ absorbs products in $R$.
      }}
    \end{itemize}

    \item \textbf{Vector space:} an abelian group of vectors equipped with scalar multiplication by elements of a field.
      \scriptsize{
      \begin{itemize}
	\item \textbf{basis}: linearly independent vectors that span the vector space\\
	  ie. for $b_1, \ldots, b_n$ a basis of $V$, then $\forall v \in V$,
	  $$v=v_1 b_1 + \ldots + v_n b_n \in V$$
      \end{itemize}
      }

    \normalsize
    \item \textbf{Module:} a generalization of a vector space where the scalars come from a ring instead of necessarily a field.
    %   \scriptsize{
    % \begin{tcolorbox}[colback=gray!15, colframe=gray!15]Intuition:
    %     Vector Spaces are defined over fields, a Module is the generalization of a vector space where the scalars come from a ring instead of a field.
    % \end{tcolorbox}}

    \normalsize
    \item \textbf{Kernel:} for a homomorphism $f:A\to B$, the subset
    $\ker(f)=\{x\in A:f(x)=0\}$ of elements mapped to zero.
      \scriptsize{
      \colorbox{gray!20}{Intuition:
       inputs that a function completely "erases", ie. turns into zero.
      }}
...
\begin{frame}{Lattices}
  ...
  \pause
  $B = \{b_1, \ldots, b_m\} \in \Rr^{n \times m}$ is the \emph{basis} of $\Ll$. We denote it $\Ll=\Ll(B)$.
  \vspace*{-0.3cm}
  $$\Ll(B) := B \cdot \Zz^n = \{ \sum_{i=1}^n c_i b_i ~|~ c_i \in \Zz^n \}$$
  \end{defn}
...
\begin{frame}{Lattice properties}
  ...
  \begin{enumerate}[i.]
    \item \emph{additive subgroup} of $\Rr^n$: $\Ll$ is the nonempty subset of $\Rr^n$ closed under vector addition (and subtraction),\\
...
\begin{frame}{q-ary lattice}
  ...
  \small{
  \begin{tcolorbox}[colback=gray!15, colframe=gray!15] Intuition:
    keep only the points that satisfy some modular rules. Since everything repeats every $q$ steps, the lattice has a structured and periodic shape, convenient in cryptography.
    \\Linear algebra modulo $q$, but breaking it requires solving a hard lattice problem.
  \end{tcolorbox}
  }
  % TODO why q-ary is better? prev.slide says O(n^2) without q-ary, how with q-ary?
  % - modular takes less bits than full integer
  % - usually A \in \Zz_1^{n \times m} with n<m
  % - membership is extremely simple (Ax (mod q))
  %
  % Notation: \perp ddenotes that the vectors are orthogonal to the rows of \(A\) modulo \(q\), i.e. they lie in the modular kernel.
\end{frame}
\end{comment}
...
\begin{frame}{Polynomial rings}
  \small{
    % \begin{itemize}
      % \item the NTRU paper (1996) was one of the first lattice-based cryptography using polynomial quotient rings, which later same general idea was ported over Ajtai/SIS-style and LWE, leading to Ring-SIS, Ring-LWE, and later on Module-LWE, etc.
      % 
    ...
    % \end{itemize}
      NTRU paper (1996), first usage of polynomial quotient rings for lattice-based cryptography. Later ported over Ajtai/SIS-style and LWE, leading to Ring-SIS, Ring-LWE, and later on Module-LWE, etc.
      
	$$
	(a_0,\ldots,a_{n-1}) \in \Zz_q^n ~~\Longleftrightarrow~~
	a(X) = a_0 + \cdots + a_{n-1} X^{n-1} \in \Zz_q[X]/(X^n+1)
	$$
  }
  ...
  \pause
	...
	\hspace*{4em}Since $X^n=-1$ then $X^{n+k} = X^n \cdot X^k= -1 \cdot X^k = -X^k$\\
...
\begin{frame}{Incomplete Timeline}
  \begin{timeline}
    ...
    1996 \quad NTRU encryption, & Ajtai
    ...
    2017 \quad Kyber (Module-LWE based), & Dilithium  (Module-LWE & Module-SIS based), & Falcon (NTRU based)
...
\begin{column}{0.76\textwidth}
  \begin{defn}[SIS (Short Integer Solution)]
    ...
    \small{"short": $z \in \{\beta, \beta\}^n$ for some bound $\beta$, often $z \in \{-1, 0, 1 \}^n$.}
  \end{defn}
\end{column}
\begin{column}{0.34\textwidth}
  \pause
  \includegraphics[width=\linewidth]{./sis-equation}
\end{column}
\end{columns}
\vspace{0.2cm}
  ...
  where the SIS problem becomes almost the SVP for $\Lambda_q^{\perp}(A)$

  \small{
    \begin{tcolorbox}[colback=gray!15, colframe=gray!15]Intuition:
    find a short nonzero vector (not the shortest as in SVP) in the lattice $\Lambda^{\perp}_q(A)$
    \end{tcolorbox}
  }
...
\begin{frame}{On the shortness of $z$}
  ...
  \end{itemize}
    ...
    \begin{tcolorbox}[colback=gray!15, colframe=gray!15]
      TL;DR: $z$ must be small enough for security reasons.
    \end{tcolorbox}
...
\begin{frame}{Hard problems - LWE}
\begin{columns}[c,onlytextwidth]
\begin{column}{0.76\textwidth}
  \begin{defn}[LWE (Learning With Errors)]
    choose a matrix $A \in^R \Zz_q^{m \times n}$, a secret vector $s \in^R \Zz_q^n$, and an error vector $e \in^R [-B, B]^m$. Let
    $$b = As +e \pmod q \in \Zz_q^m$$
...
\begin{column}{0.34\textwidth}
  \pause
  \includegraphics[width=\linewidth]{./lwe-equation}
\end{column}
\end{columns}
...
\begin{frame}
  \centering
  \includegraphics[width=8.5cm]{./lwe-sphere-error}
\end{frame}
...
% TODO maybe remove dual lattices slide
\begin{comment}
\begin{mayberm}
\begin{frame}{Dual lattices}
  ...
  \small{(duality is interesting, because it lets security proofs transfer geometric properties)}

  % TODO add intuition explainer on dual lattices
  % TODO fix fitting everything
    \scriptsize{
  \begin{tcolorbox}[colback=gray!15, colframe=gray!15]Intuition:
    A lattice tells you where points are allowed to sit; the dual lattice tells you which periodic directions “fit perfectly” with that grid.
    Original lattice describes the spacing of points, while the dual lattice describes the spacing of compatible frequencies. Wide spacing in the original means tighter spacing in the dual, and vice versa.
  \end{tcolorbox}}
\end{frame}
\end{mayberm}
\end{comment}
...
\begin{frame}{Lattice-based cryptography}
  ...
  \vspace{1.0cm}
  \pause
  Let's do a quick overview of ML-DSA (Dilithium)...
\end{frame}
...
% \begin{frame}{Recall: Sigma protocol over elliptic curve}
% \begin{center}
...
\vspace*{-1.8cm}
$$\textcolor{red}{x} \in \mathbb{Z}_q, ~ \textcolor{blue}{Y}=\textcolor{red}{x}G$$
$$\textcolor{red}{r} \leftarrow \mathbb{Z}_q, ~~ \textcolor{blue}{R}=\textcolor{red}{r}G$$

$$\textcolor{blue}{z} =\textcolor{red}{r}+c\textcolor{red}{x} \pmod q$$
...
\begin{sequencediagram}
    ...
    % \mess[0]{a}{$\textcolor{blue}{R}$}{b}
    \mess[0]{a}{$R$}{b}
    \mess[2]{b}{$c$}{a}
    % \mess[2]{a}{$\textcolor{blue}{z}$}{b}
    \mess[2]{a}{$z$}{b}
    \mess[0]{b}{$ok$}{a}
\end{sequencediagram}
}
...
% Move only this equation down, without affecting the frame layout
\raisebox{-1.4cm}[0pt][0pt]{%
  \makebox[\linewidth][c]{$c \leftarrow \mathbb{Z}_q$}%
}


\vspace*{2.5cm}
$$\textcolor{blue}{z}G \stackrel{?}{=} \textcolor{blue}{R}+c\textcolor{blue}{Y}$$

indeed,
\begin{align*}
  \textcolor{blue}{z}G &= (\textcolor{red}{r}+c\textcolor{red}{x})G \\
   &= \textcolor{red}{r}G+c(\textcolor{red}{x}G) \\
   &= \textcolor{blue}{R}+c\textcolor{blue}{Y}
\end{align*}
...
\begin{column}{0.27\textwidth}
\vspace*{-1.8cm}
$$\textcolor{red}{s_1} \leftarrow S^\ell,~ \textcolor{red}{s_2} \leftarrow S^k$$
  $$\textcolor{blue}{A} \leftarrow R_q^{k\times\ell},~ \textcolor{blue}{t} = \textcolor{blue}{A}s_1+s_2$$
  $$\textcolor{red}{y_1}\leftarrow Y^\ell,~ \textcolor{red}{y_2}\leftarrow Y^k$$
  $$\textcolor{blue}{w} = \textcolor{blue}{A}\textcolor{red}{y_1}+\textcolor{red}{y_2}$$

  $$\textcolor{blue}{z_1}=\textcolor{red}{y_1}+c\textcolor{red}{s_1},~ \textcolor{blue}{z_2}=\textcolor{red}{y_2}+c\textcolor{red}{s_2}$$
  Reject if $z_1,z_2$ are not short.
\end{column}
...
\begin{sequencediagram}
    ...
    \mess[0]{a}{$w$}{b}
    \node[anchor=west] at (6.0,-1.5) {
	$c\leftarrow C$
	(in practice, $c = H(M ||w)$)
    };
    \mess[2]{b}{$c$}{a}
    \mess[2]{a}{$z_1,z_2$}{b}
    % \node[anchor=west] at (7.0,-5.0) {
    %     $w' = Az_1 + z_2 - ct$
    % };
    \mess[0]{b}{$ok$}{a}
\end{sequencediagram}
...
\vspace*{2.5cm}
  \begin{align*}
  \textcolor{blue}{w} &\stackrel{?}{=} \textcolor{blue}{Az_1}+\textcolor{blue}{z_2}-c \textcolor{blue}{t} \\
      &\text{indeed,}\\
      &= \textcolor{blue}{A}(\textcolor{red}{y_1}+c\textcolor{red}{s_1})+(\textcolor{red}{y_2}+c\textcolor{red}{s_2}) \\
      &~-c(\textcolor{blue}{A} \textcolor{red}{s_1}+\textcolor{red}{s_2}) \\
    =\textcolor{blue}{A} &\textcolor{red}{y_1} +c \textcolor{blue}{A} \textcolor{red}{s_1} +\textcolor{red}{y_2} +c \textcolor{red}{s_2}-c\textcolor{blue}{A} \textcolor{red}{s_1}-c\textcolor{red}{s_2}\\
			 &= \textcolor{blue}{A} \textcolor{red}{y_1}+\textcolor{red}{y_2} = \textcolor{blue}{w}
  \end{align*}
  Accept if $z_1,z_2$ are short.
...
%   $s_1 \leftarrow S^\ell,~ s_2 \leftarrow S^k$
%   \\[1mm]
%   Public:\\
%   $A \leftarrow R_q^{k\times\ell},~ t = As_1+s_2$
% };
% 
% \node[anchor=west, align=left] at (8.3,-1.4) {
%   Public key:\\
%   $(A,t)$
% };
% 
...
% % Commitment
% 
% \node[anchor=west, align=left] at (0,-2.35) {
%   Sample short vectors:\\
%   $y_1\leftarrow Y^\ell,~
%    y_2\leftarrow Y^k$
%   \\[1mm]
%   Compute:\\
...
% % Challenge
% 
% \node[anchor=west, align=left] at (8.3,-3.25) {
%   $c\leftarrow C$\\
%   \scriptsize{
%     (in practice, $c = H(M ||w)$)
%   }
...
% % Response
% 
% \node[anchor=west, align=left] at (0,-4.45) {
%   Compute:\\
%   $z_1=y_1+cs_1,~
%    z_2=y_2+cs_2$
%   \\[1mm]
...
% % Verification
% 
% \node[anchor=west, align=left] at (8.3,-6.0) {
%   Reconstruct:\\[1mm]
%   $\begin{aligned}
%   w'
%     &= Az_1+z_2-ct \\
...
\begin{frame}{Benchmarks}
  ...
  \vspace*{0.5cm}
  \tiny{\emph{*stateful: signer needs to keep a state, keeping track of the leaves already used. Fine for validator nodes, potential trouble for wallets.}}
\end{frame}
...
\begin{frame}{}
  ...
  \begin{itemize}
    \item quantum thread forces us to prepare for it. Most companies are already preparing for migration to post-quantum
      \pause
    \item no tool (scheme) for all use cases (eg. Ethereum validators vs wallets)
      \pause
    \item lattice-based cryptography:

    \scriptsize{
    \begin{itemize}
      \item not a single assumption on all schemes: NTRU, R-SIS, M-SIS, R-LWE
      \item actual operations (implementation) are over polynomial rings
      % \item smaller proofs and keys than hash-based
      \item often significantly smaller signatures/proofs than hash-based alternatives, at the cost of more algebraic complexity
      \item often we're interested in 'short' values $\longrightarrow$ typical trick: decompose bigger values before operate on them
    \end{itemize}
    }
    \small

    \pause
    \item what to read from here
    \begin{itemize}
      \item \emph{A Decade of Lattice Cryptography}, Chris Peikert \href{https://eprint.iacr.org/2015/939}{https://eprint.iacr.org/2015/939}
      \item \emph{A gentle introduction to lattice-based cryptography}, Alfred Menezes \href{https://eprint.iacr.org/2026/1098}{https://eprint.iacr.org/2026/1098}
    \end{itemize}
  \end{itemize}
