...
Let $q$ be a prime power. View $(Fq)^n$ as the vector space $\Vnq$.
...
Let $C$ a $[n,k]$-code over $GF(q)$ with GM $G$. $C$ contains $q^k$ codewords $\Longrightarrow$ can be used to comunicate any one of $q^k$ distinct messages.
...
\begin{defn}{}[Capacity]
  the \emph{capacity} $\mathfrak{C}(p)$ of a binary symmetric channel with symbol error probability $p$ is
  $$\mathfrak{C}(p) = 1+ p \cdot \log_2{(p)} + (1-p) \cdot \log_2{(1-p)}$$
\end{defn}
